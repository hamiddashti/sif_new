function m = model_fun_dynamic(v)
...
if strcmp(alpha_opt,"dynamic") == 1
    gm = 1/rm;
    solA = dynamic_solver.eqA(Abs,CB6F,c_steps,Ca,E,Kc,Kd,Kf,Ko,Kp1,O,Q,Rd,gm,gtc,kc,ko,kq,nc,nl);
    solB = dynamic_solver.eqB(Abs,CB6F,c_steps,Kc,Kd,Kf,Ko,Kp1,Kp2,Ku2,O,Q,kc,ko,kq,nc,nl);
    tmp  = abs(solA-solB);
    [~, I] = min(tmp,[],2);
    ind = sub2ind(size(solA),(1:length(Q))',I);
    a2 = solA(ind);
    a1 = Abs - a2;
     ch = c_steps(I)';
end

if strcmp(alpha_opt,"free") == 1
    a2 = v.a2;
    a1 = Abs - a2;
end
